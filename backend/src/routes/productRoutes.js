const express = require('express');
const router = express.Router();
const productController = require('../controllers/productController');
const upload = require('../middlewares/uploadMiddleware');

// Khai báo các đường dẫn API tương ứng với Controller
router.get('/', productController.getAllProducts);
router.post('/', upload.single('hinhAnh'), productController.createProduct);
router.put('/:id', upload.single('hinhAnh'), productController.updateProduct);

// Dùng method PATCH vì ta chỉ cập nhật 1 phần nhỏ (trạng thái) của sản phẩm
router.patch('/:id/deactivate', productController.deactivateProduct); 

// Các API Dịch vụ
router.get('/services', productController.getAllServices);
router.post('/services', productController.createService);
router.put('/services/:id', productController.updateService);
router.patch('/services/:id/deactivate', productController.deactivateService);
router.delete('/services/:id', productController.deleteService);

module.exports = router;